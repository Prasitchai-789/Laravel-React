...
class YieldTableController extends Controller
{
    ...
    /**
     * ดึงข้อมูลสำหรับตารางในลักษณะ Loop สรุปรายเดือน (API)
     */
    public function apiData(Request $request)
    {
        try {
            $month = $request->query('month', now()->format('Y-m'));
            $tableData = $this->getCalculatedYieldData($month);
    ...
    public function export(Request $request)
    {
        ...
        $data = $this->getCalculatedYieldData($monthStr);

        return \Maatwebsite\Excel\Facades\Excel::download(
            new \App\Exports\QAC\YieldTableExport($data, $month),
            "Yield_Report_{$monthStr}.xlsx"
        );
    }

    /**
     * Logic หลักในการคำนวณข้อมูล Yield รายวัน
     */
    private function getCalculatedYieldData($month)
    {
