...
export default function StoreOrderDetail({ order, onClose, showHistory = true }: Props) {
    ...
    const calculateHistoryTotals = (
        ...
        console.log('--- calculateHistoryTotals ---');
        console.log('Product ID:', productId);
        console.log('Item quantity:', itemQty);
        console.log('Document date:', docuDate);
        console.log('History raw:', history);

        if (!productId) return { issuedFromHistory: 0, returnedFromHistory: 0, totalIssued: itemQty, pendingQty: itemQty };

        // กรองเฉพาะ product_id ของ item นี้ และหลังเอกสาร
        const filteredHistory = history
            .filter(h => h.product_id === productId)
            .filter(h => new Date(h.docu_date) > new Date(docuDate));

        console.log('Filtered history for this item:', filteredHistory);

        const issuedFromHistory = filteredHistory
            .filter(h => h.movement_type === 'เบิก')
            .reduce((sum, h) => sum + Number(h.quantity), 0);

        const returnedFromHistory = filteredHistory
            .filter(h => h.movement_type === 'คืน')
            .reduce((sum, h) => sum + Number(h.quantity), 0);

        const totalIssued = Number(itemQty);
        const pendingQty = Math.max(0, totalIssued + issuedFromHistory - returnedFromHistory);

        console.log('Issued:', issuedFromHistory, 'Returned:', returnedFromHistory, 'Total issued:', totalIssued, 'Pending:', pendingQty);
                                    ...
                                    <table className="w-full text-sm">
                                        ...
                                        <tbody className="divide-y divide-gray-100">
                                            {order.items.map((item) => {
                                                // ใช้ docu_date ของเอกสารนี้ เพื่อกรอง history หลังเอกสาร
                                                    ...
                                                    <React.Fragment key={item.id}>
                                                        ...
                                                        <tr className="hover:bg-blue-50/50 transition-colors duration-150 group">
                                                            ...
                                                            <td className="px-4 py-3 text-center text-gray-600">
                                                                <span className="bg-gray-100 px-2 py-1 rounded-md text-xs">{item.unit || '-'}</span>
                                                            </td>
                                                                ...
                                                                <>
                                                                    ...
                                                                    <td className="px-4 py-3 text-center">
                                                                        <div className="flex items-center justify-center">
                                                                            ...
                                                                            <span
                                                                                className={`font-bold px-2 py-1 rounded-md ${pendingQty > 0
                                                        ...
                                                        {isExpanded && hasHistory && (
                                                            <>
                                                                ...
                                                                {item.history?.map((h, idx) => (
                                                                    <tr
                                                                        key={`${item.id}-${idx}`}
                                                                        className={`transition-colors duration-150 ${h.movement_type === 'เบิก'
                                                                    ...
                                                                    >
                                                                        <td className="px-4 py-2 text-gray-700 text-sm font-medium pl-8" colSpan={2}>
                                                                            <div className="flex items-center">
                                                                                <div
                                                                                    className={`w-2 h-2 rounded-full mr-2 ${h.movement_type === 'เบิก' ? 'bg-orange-500' : 'bg-green-500'
                                                                                        }`}
                                                                                ></div>
                                                                        ...
                                                                        <td className="px-4 py-2 text-center">
                                                                            <span
                                                                                className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium ${h.movement_type === 'เบิก'
                                                                        ...
                                                                        {showHistory && (
                                                                            <>
                                                                                ...
                                                                                <td className="px-4 py-2 text-center text-gray-600 text-sm font-medium">{h.user_id}</td>
                                                                            </>
                                                                        )}
                                                                    </tr>
                                                                ))}
